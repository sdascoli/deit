...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, num_patches=197, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., use_local_init=True):
        super().__init__()
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads,num_patches=197, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
...
class VisionTransformer_no_loc(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
...
@register_model
def vit_base_patch16_224_no_loc(pretrained=False, **kwargs):
    model = VisionTransformer_no_loc(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = default_cfgs['vit_base_patch16_224']
    if pretrained:
